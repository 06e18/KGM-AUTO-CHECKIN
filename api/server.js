...
// ========== 日志脱敏（避免 token/手机号/userid 等敏感信息写入 Actions 日志）==========
const SENSITIVE_QUERY_KEYS = new Set([
  'token', 'vip_token', 'viptoken', 'cookie', 'authorization',
  'code', 'mobile', 'phone', 'qrcode', 'qrcode_img', 'qrcode_txt',
  'qrimg', 'key', 'p', 'p2', 'p3', 'params',
]);
...
const SENSITIVE_BODY_KEYS = new Set([
  ...SENSITIVE_QUERY_KEYS, 'pat', 'gh_token', 'userinfo', 'password',
]);
...
function sanitizeUrlForLog(rawUrl) {
  ...
  if (queryStart === -1) return decodedUrl;
...
function sanitizeBodyForLog(value, depth = 0) {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, item]) => {
      const normalizedKey = key.toLowerCase();
      if (DISPLAY_NAME_KEYS.has(normalizedKey)) return [key, maskDisplayNameForLog(item)];
      if (IDENTIFIER_QUERY_KEYS.has(normalizedKey)) return [key, maskIdentifierForLog(item)];
      if (SENSITIVE_BODY_KEYS.has(normalizedKey)) return [key, '[REDACTED]'];
...
async function consturctServer(moduleDefs) {
  ...
  for (const moduleDef of moduleDefinitions) {
    ...
    app.use(moduleDef.route, async (req, res) => {
      ...
      const query = Object.assign({}, { cookie: Object.assign({}, req.cookies, cookieSource) }, params, body);
